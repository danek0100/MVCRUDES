...
MySqlCrudController::MySqlCrudController(std::shared_ptr<ITypesConverter> typesConverter) :
                                                                            typesConverter(std::move(typesConverter)) {}

MySqlCrudController::~MySqlCrudController() = default;

// TODO simplify in future for testing
std::list<JsonEntity> MySqlCrudController::loadEntities(const std::string& tableName, const Json::Value& filter, const std::set<std::string>& fields = {}) {
...
// TODO simplify in future for testing
void MySqlCrudController::addEntities(std::list<JsonEntity>& objects) {
    TRACE(__FUNCTION__ << "(). Entry point.");
    ...
    for (auto& typedEntity : typeSeparation) {
        ...
        for (auto& object: typedEntity.second) {
            if (object->key == 0) { PRINTLN(__FUNCTION__ << "() found 0 key"); has0Key = true; }
        ...
        for (auto& entities: fieldsSets) {
            ...
            for (const auto& pair: entities.second.front()->value) {
            ...
            try {
                if (has0Key) {
                    client.get()->update("SET @OLD_SQL_MODE=@@SQL_MODE;");
                    client.get()->update("SET SESSION SQL_MODE = 'NO_AUTO_VALUE_ON_ZERO';");
                }

                client.get()->update(query.str());

                if (has0Key) {
                    client.get()->update("SET SESSION SQL_MODE = @OLD_SQL_MODE;");
                }

                if (entities.second.front()->key < 0) {
                    int lastId = client.get()->queryLastAutoId(typedEntity.first.c_str());

                    int firstId = lastId - (int)entities.second.size() + 1;
                    for (auto& object: entities.second) {
                        object->key = firstId++;
...
// TODO simplify in future for testing
std::list<JsonEntity> MySqlCrudController::updateEntities(std::list<JsonEntity>& objects) {
    SqlPoolClient client;
    std::list<JsonEntity> result;
    ...
    for (auto& typedEntity : typeSeparation) {
        ...
        for (auto& entities : fieldsSets) {
            ...
            std::map<std::string, std::stringstream> updates;
            for (const auto& object : entities.second) {
...
// TODO simplify in future for testing
std::list<JsonEntity> MySqlCrudController::removeEntities(std::list<JsonEntity>& objects) {
...
std::string MySqlCrudController::buildSelectWhereClause(const std::string& entityName, const Json::Value& filter) {
...
std::string MySqlCrudController::buildSelectOrderByClause(const Json::Value& filter){
    TRACE(__FUNCTION__ << "(): " << "entry point")
...
std::string MySqlCrudController::join(const std::vector<std::string>& elements, const std::string& connector) {
...
std::string MySqlCrudController::getSqlOperator(const std::string& type) {
